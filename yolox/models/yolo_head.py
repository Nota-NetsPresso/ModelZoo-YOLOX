...
class YOLOXHead_1(nn.Module):
    ...
    def forward(self, xin):
        ...
        for k, (cls_conv, reg_conv, x) in enumerate(
            ...
            reg_list.append(reg_output)
            obj_list.append(obj_output)
            
        return xin[0], cls_list, reg_list, obj_list
...
class YOLOXHead_2(nn.Module):
    ...
    def forward(self, inputs, labels=None, imgs=None):
        ...
        xin_0, cls_list, reg_list, obj_list = inputs
        ...
        for k, (cls_output, reg_output, obj_output, stride_this_level) in enumerate(zip(cls_list, reg_list, obj_list, self.strides)):
            if self.training:
                output = torch.cat([reg_output, obj_output, cls_output], 1)
                output, grid = self.get_output_and_grid(
                    output, k, stride_this_level, xin_0.type()
                )
                x_shifts.append(grid[:, :, 0])
                y_shifts.append(grid[:, :, 1])
                expanded_strides.append(
        ...
        if self.training:
            ...
            if self.decode_in_inference:
                return self.decode_outputs(outputs, dtype=xin_0.type())
            else:
                return outputs
    ...
    def visualize_assign_result(self, inputs, save_prefix="assign_vis_"):
        xin_0, labels, imgs, cls_list, reg_list, obj_list = inputs
        # original forward logic
        outputs, x_shifts, y_shifts, expanded_strides = [], [], [], []
        # TODO: use forward logic here.
        ...
        for k, (cls_output, reg_output, obj_output, stride_this_level) in enumerate(zip(cls_list, reg_list, obj_list, self.strides)):
            output = torch.cat([reg_output, obj_output, cls_output], 1)
            output, grid = self.get_output_and_grid(output, k, stride_this_level, xin_0.type())
            x_shifts.append(grid[:, :, 0])
            y_shifts.append(grid[:, :, 1])
            expanded_strides.append(
                torch.full((1, grid.shape[1]), stride_this_level).type_as(xin_0)
            )
            outputs.append(output)
